# weather_etl.py — pull hourly weather for each ride window and write to Google Sheets (chunked writes + progress)
...
# Tuning / limiting for test runs (set via GitHub Action env or local env)
...
def build_weather(sh):
    ...
    total = len(elig)
    if total == 0:
        print(f"{PRINT_PREFIX} no activities with usable start/elapsed/latlon in scope.")
        return pd.DataFrame(), pd.DataFrame()

    # Prepare tabs immediately so you can see them
    ...
    for i, (_, row) in enumerate(elig.iterrows(), start=1):
        ...
        if i % WX_FLUSH_EVERY == 0:
            # Write partial results so you can see progress
            df_hourly_so_far = pd.concat(hourly_rows, ignore_index=True) if hourly_rows else pd.DataFrame(columns=hourly_headers)
            df_agg_so_far    = pd.DataFrame(agg_rows, columns=agg_headers) if agg_rows else pd.DataFrame(columns=agg_headers)
            write_df_to_tab(sh, TAB_WX_HOURLY, df_hourly_so_far)
            write_df_to_tab(sh, TAB_WX_BY_RIDE, df_agg_so_far)
            print(f"{PRINT_PREFIX} progress: wrote partial results at {i}/{total} rides.")

        print(f"{PRINT_PREFIX} activity {i}/{total} processed (id={aid}).")
        time.sleep(THROTTLE_S)

    # Final write
    df_hourly = pd.concat(hourly_rows, ignore_index=True) if hourly_rows else pd.DataFrame(columns=hourly_headers)
    df_by_ride = pd.DataFrame(agg_rows, columns=agg_headers) if agg_rows else pd.DataFrame(columns=agg_headers)
    return df_hourly, df_by_ride

def main():
    print(f"{PRINT_PREFIX} START weather ETL")
    if WX_SINCE:
        print(f"{PRINT_PREFIX} filter WX_SINCE={WX_SINCE}")
    if WX_MAX_RIDES:
        print(f"{PRINT_PREFIX} cap WX_MAX_RIDES={WX_MAX_RIDES}")
    print(f"{PRINT_PREFIX} flush every {WX_FLUSH_EVERY} rides")
    ...
    # Write final state (in case last chunk wasn't a multiple of WX_FLUSH_EVERY)
